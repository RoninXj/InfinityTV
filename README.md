...
本项目是在 **MoonTV** 基础上进行的深度二次开发版本，从 **v4.3.1** 版本开始，持续迭代至当前 **v5.7.1**，累计新增 50+ 重大功能模块，300+ 细节优化。所有新增功能详见 [CHANGELOG](CHANGELOG)。
...
### 最新版本：v5.7.1 (2025-12-13)

#### 新增功能
- 🎨 Liquid-glass 毛玻璃效果播放器控制栏：添加现代化的毛玻璃效果控制栏，提升视觉体验
- 💬 豆瓣评论集成：播放页面展示豆瓣用户评论，提供更丰富的影片信息
- ❤️ 全局收藏功能：支持跨设备的全局收藏功能，收藏数据同步到数据库
- 📋 收藏分类管理：收藏页面支持按类型筛选（电影、剧集、综艺、短剧、番剧）
- 🎬 备用 API 支持：搜索和首页数据加载支持备用 API，提高系统稳定性
- 🔢 完结系列集数统计：搜索和分类页面显示完结系列的集数统计
- 📱 移动端弹幕控制栏优化：优化移动端弹幕控制栏的显示和交互

#### 优化改进
- ⚡ 性能优化：优化搜索和首页数据加载性能，减少不必要的 API 请求
- 📊 分类显示优化：改进分类和类型信息的显示方式
- 🎯 API 错误处理：增强 API 错误处理和备用方案
- 🖼️ 海报显示改进：优化海报图片的显示效果
- 🔄 数据加载优化：改进数据缓存和加载机制

#### Bug 修复
- 🎮 修复播放器控制栏按钮溢出问题：通过 flex-shrink 和自适应宽度解决移动端按钮溢出
- 🎮 修复控制栏拖动进度条时的漂移问题：移除控制栏事件监听，完全由 CSS 控制宽度
- 📂 修复分类过滤问题：修复分类筛选功能的显示错误
- 🏷️ 修复类型显示错误：修正影片类型信息的显示逻辑
- 🔢 修复集数统计问题：修复完结系列集数统计的准确性
- 🔧 修复备用 API 逻辑：完善备用 API 的切换和错误处理逻辑
